...
function ItemNote({ productId, note, onSave }) {
  ...
  if (!editing && note) {
    return (
      <button
        ...
        className="mt-1 text-left text-xs text-ink-muted italic hover:text-pine"
      >
        Note: {note}
      </button>
    );
  ...
  if (!editing) {
    return (
      <button
        ...
        className="mt-1 flex items-center gap-1 text-xs font-medium text-pine hover:text-pine-deep"
      >
  ...
  return (
    <div className="mt-1 flex items-center gap-2">
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter") { onSave(productId, draft); setEditing(false); }
          if (e.key === "Escape") setEditing(false);
        }}
      />
      <button
        className="text-xs font-semibold text-pine"
        onClick={() => { onSave(productId, draft); setEditing(false); }}
      >
        Save
      </button>
    </div>
...
export default function Cart({ role }) {
  ...
  const handleNoteChange = async (productId, note) => {
    try {
      const updated = await updateCartItem(productId, { note });
      setCart(updated);
    } catch (err) {
      setError(err.message);
    }
      ...
      {!cart || cart.items.length === 0 ? (
        ...
        <>
          <div className="mt-4 space-y-3">
            {cart.items.map((item) => (
              <Card key={item.product?.id || Math.random()} className="p-4">
                {item.unavailable || !item.product ? (
                  <div>
                    <div className="font-semibold text-ink-muted line-through">Item no longer available</div>
                    <button className="mt-2 text-sm font-semibold text-clay" onClick={() => handleRemove(item.product?.id)}>
                      Remove
                    </button>
                  </div>
                ) : (
                    ...
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          ...
                          className="flex h-9 w-9 items-center justify-center rounded-lg bg-paper text-ink hover:bg-line transition-colors"
                        >
                          <Minus size={16} />
                        </button>
                      ...
                      <div className="w-16 shrink-0 text-right font-semibold text-ink">{formatPrice(item.subtotal)}</div>
                      <button aria-label="Remove item" className="shrink-0 text-ink-muted hover:text-clay transition-colors" onClick={() => handleRemove(item.product.id)}>
                        <X size={18} />
                      </button>
                    </div>
                    <ItemNote productId={item.product.id} note={item.note} onSave={handleNoteChange} />
                  </div>
                )}
              </Card>
            ))}
          ...
          <div className="mt-6 flex items-center justify-between border-t border-line pt-4">
            <button onClick={handleClear} className="text-sm font-semibold text-ink-muted hover:text-clay transition-colors">
              Clear cart
            </button>
            <div className="text-xl font-bold text-ink">Total: {formatPrice(cart.total)}</div>
          </div>
